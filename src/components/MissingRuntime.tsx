...
// Shown when Logged is opened outside General Text — no injected `window.gt`
// runtime (visiting the deployed site directly). A General Text app has no backend
// of its own, so on its own there's nothing to read or write; point the visitor at
// how to use it, and let them try a local sample-data demo right here.
export function MissingRuntime({ onTryDemo }: { onTryDemo: () => void }) {
  const appUrl = typeof window !== 'undefined' ? window.location.origin : 'https://logged.generaltext.org'
    ...
    <div className="flex min-h-full items-center justify-center px-6 py-12" style={{ background: 'var(--bg)', color: 'var(--fg)' }}>
      <div className="w-full max-w-md space-y-5 rounded-2xl border p-7 shadow-sm" style={{ borderColor: 'var(--border)', background: 'var(--panel)' }}>
        ...
        <p className="text-sm leading-relaxed" style={{ color: 'var(--muted)' }}>
          Logged runs <span style={{ color: 'var(--fg)', fontWeight: 500 }}>inside General Text</span>, a workspace
        ...
        <div className="space-y-2">
          <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--muted)' }}>To use Logged</p>
          <ol className="space-y-1.5 text-sm" style={{ color: 'var(--muted)' }}>
            ...
            <Step n={4}>Launch Logged from your workspace.</Step>
          </ol>
        </div>
        ...
        <p className="-mt-1 text-xs" style={{ color: 'var(--muted)' }}>
          The demo loads a sample log locally in your browser. Nothing is saved to an account, and changes
          stay on this device.
        </p>
