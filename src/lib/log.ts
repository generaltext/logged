// The append-only log: monthly JSONL shards under v0/log/. Helpers here are
...
const SHARD_RE = new RegExp(`^${DATA_VERSION}/log/\\d{4}-\\d{2}\\.jsonl$`)

export function shardForDate(d: Date): string {
  const y = d.getFullYear()
  const m = String(d.getMonth() + 1).padStart(2, '0')
  return `${LOG_DIR}/${y}-${m}.jsonl`
}
...
/**
 * Fold the not-yet-consumed tail of a shard into state. Returns the new consumed
